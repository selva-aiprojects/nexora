...
/**
 * SQLite-backed implementation of the Store seam (node:sqlite, synchronous).
 *
 * Data is stored as a JSON-document table keyed by (tenant_id, collection,
 * id). This keeps the exact synchronous API the services already use, so
 * swapping the in-memory store for a real database required zero changes to
 * module code. Tenant isolation is enforced at the query layer via the
 * `tenant_id` column (the application-layer half of the PRD's requirement;
 * the database-layer half — PostgreSQL RLS — lives in infra/postgres).
 */
export class SqliteStore implements Store {
  ...
  private readRows(tenantId: string, name: string): any[] {
    const rows = this.db
  ...
  update(tenantId: string, name: string, id: string, patch: any): any {
    const current = this.byId(tenantId, name, id);
  ...
  query(tenantId: string, name: string, predicate: (row: any) => boolean): any[] {
    return this.readRows(tenantId, name).filter(predicate);
  }
