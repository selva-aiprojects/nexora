...
// ----------------------------- Routes -----------------------------
router.get('/categories', requireAuth, asyncHandler(async (_req, res) => res.json(await db.all(TID, COL.categories))));
router.get('/obligations', requireAuth, asyncHandler(async (_req, res) => res.json(await db.all(TID, COL.obligations))));
router.get('/deadlines', requireAuth, asyncHandler(async (req, res) => {
  let rows = await db.all(TID, COL.deadlines);
...
router.get('/filings', requireAuth, asyncHandler(async (req, res) => {
  let rows = await db.all(TID, COL.filings);
...
router.post('/filings/:id/submit', requireAuth, requireRole('finance', 'hr', 'admin', 'owner'), asyncHandler(async (req, res) => {
  const filing = notFoundIfUndefined(await db.byId(TID, COL.filings, req.params.id), 'Filing not found');
  const updated = await db.update(TID, COL.filings, filing.id, { status: 'submitted', filedOn: new Date().toISOString().slice(0, 10) });
  const a = actor(req); await recordAudit({ tenantId: TID, actorId: a.id, actorName: a.name, action: 'create', module: 'compliance', recordRef: filing.id, newState: { status: 'submitted' }, ip: req.ip });
  res.json(updated);
}));
router.post('/filings/:id/verify', requireAuth, requireRole('admin', 'owner'), asyncHandler(async (req, res) => {
  const filing = notFoundIfUndefined(await db.byId(TID, COL.filings, req.params.id), 'Filing not found');
  const updated = await db.update(TID, COL.filings, filing.id, { status: 'verified' });
  const a = actor(req); await recordAudit({ tenantId: TID, actorId: a.id, actorName: a.name, action: 'update', module: 'compliance', recordRef: filing.id, newState: { status: 'verified' }, ip: req.ip });
  res.json(updated);
}));
router.post('/evidence', requireAuth, asyncHandler(async (req, res) => {
  requireBody(req.body, ['filingId', 'fileName']);
  const row = await db.insert(TID, COL.evidence, { id: await db.nextId('cve', COL.evidence), uploadedOn: new Date().toISOString().slice(0, 10), ...req.body });
  const a = actor(req); await recordAudit({ tenantId: TID, actorId: a.id, actorName: a.name, action: 'create', module: 'compliance', recordRef: row.id, newState: row, ip: req.ip });
  res.status(201).json(row);
}));
router.get('/evidence/:filingId', requireAuth, asyncHandler(async (req, res) => {
  res.json(await db.query(TID, COL.evidence, (e) => e.filingId === req.params.filingId));
}));

async function init() {
  await db.seed(COL.categories, [
