...
router.get('/customers', requireAuth, asyncHandler(async (req, res) => {
  let rows = await db.all(TID, COL.customers);
...
router.post('/customers', requireAuth, requireRole('manager', 'admin', 'owner'), asyncHandler(async (req, res) => {
  ...
  const row = await db.insert(TID, COL.customers, { id: await db.nextId('cust', COL.customers), createdAt: new Date().toISOString().slice(0, 10), ...req.body });
  const a = actor(req); await recordAudit({ tenantId: TID, actorId: a.id, actorName: a.name, action: 'create', module: 'crm', recordRef: row.id, newState: row, ip: req.ip });
  res.status(201).json(row);
}));
router.get('/customers/:id', requireAuth, asyncHandler(async (req, res) => {
  const customer = notFoundIfUndefined(await db.byId(TID, COL.customers, req.params.id), 'Customer not found');
  res.json(customer);
}));
router.put('/customers/:id', requireAuth, requireRole('manager', 'admin', 'owner'), asyncHandler(async (req, res) => {
  const existing = notFoundIfUndefined(await db.byId(TID, COL.customers, req.params.id), 'Customer not found');
  const updated = await db.update(TID, COL.customers, existing.id, req.body);
  const a = actor(req); await recordAudit({ tenantId: TID, actorId: a.id, actorName: a.name, action: 'update', module: 'crm', recordRef: existing.id, newState: updated, ip: req.ip });
  res.json(updated);
}));

// Contacts
router.get('/customers/:customerId/contacts', requireAuth, asyncHandler(async (req, res) => {
  const rows = await db.query(TID, COL.contacts, (c) => c.customerId === req.params.customerId);
  res.json(listResult(rows, rows.length, 1, rows.length));
}));
router.post('/customers/:customerId/contacts', requireAuth, requireRole('manager', 'admin', 'owner'), asyncHandler(async (req, res) => {
  ...
  const row = await db.insert(TID, COL.contacts, { customerId: req.params.customerId, ...req.body });
  const a = actor(req); await recordAudit({ tenantId: TID, actorId: a.id, actorName: a.name, action: 'create', module: 'crm', recordRef: row.id, newState: row, ip: req.ip });
  res.status(201).json(row);
}));
...
router.get('/leads', requireAuth, asyncHandler(async (req, res) => {
  let rows = await db.all(TID, COL.leads);
...
router.post('/leads', requireAuth, requireRole('manager', 'admin', 'owner'), asyncHandler(async (req, res) => {
  ...
  const row = await db.insert(TID, COL.leads, { createdAt: new Date().toISOString().slice(0, 10), ...req.body });
  const a = actor(req); await recordAudit({ tenantId: TID, actorId: a.id, actorName: a.name, action: 'create', module: 'crm', recordRef: row.id, newState: row, ip: req.ip });
  res.status(201).json(row);
}));
router.put('/leads/:id', requireAuth, requireRole('manager', 'admin', 'owner'), asyncHandler(async (req, res) => {
  const existing = notFoundIfUndefined(await db.byId(TID, COL.leads, req.params.id), 'Lead not found');
  const updated = await db.update(TID, COL.leads, existing.id, req.body);
  const a = actor(req); await recordAudit({ tenantId: TID, actorId: a.id, actorName: a.name, action: 'update', module: 'crm', recordRef: existing.id, newState: updated, ip: req.ip });
  res.json(updated);
}));
...
router.get('/quotes', requireAuth, asyncHandler(async (req, res) => {
  let rows = await db.all(TID, COL.quotes);
...
router.post('/quotes', requireAuth, requireRole('manager', 'admin', 'owner'), asyncHandler(async (req, res) => {
  ...
  const row = await db.insert(TID, COL.quotes, req.body);
  const a = actor(req); await recordAudit({ tenantId: TID, actorId: a.id, actorName: a.name, action: 'create', module: 'crm', recordRef: row.id, newState: row, ip: req.ip });
  res.status(201).json(row);
}));
router.put('/quotes/:id', requireAuth, requireRole('manager', 'admin', 'owner'), asyncHandler(async (req, res) => {
  const existing = notFoundIfUndefined(await db.byId(TID, COL.quotes, req.params.id), 'Quote not found');
  const updated = await db.update(TID, COL.quotes, existing.id, req.body);
  const a = actor(req); await recordAudit({ tenantId: TID, actorId: a.id, actorName: a.name, action: 'update', module: 'crm', recordRef: existing.id, newState: updated, ip: req.ip });
  res.json(updated);
}));
router.post('/quotes/:id/convert', requireAuth, requireRole('manager', 'admin', 'owner'), asyncHandler(async (req, res) => {
  const quote = notFoundIfUndefined(await db.byId(TID, COL.quotes, req.params.id), 'Quote not found');
  if (quote.status === 'accepted') {
    const salesOrders = await db.all(TID, COL.salesOrders);
    const number = `SO-${new Date().getFullYear()}-${String(salesOrders.length + 1).padStart(3, '0')}`;
    const so = await db.insert(TID, COL.salesOrders, {
      id: await db.nextId('so', COL.salesOrders),
    ...
    await db.update(TID, COL.quotes, quote.id, { status: 'converted' });
    const a = actor(req); await recordAudit({ tenantId: TID, actorId: a.id, actorName: a.name, action: 'create', module: 'crm', recordRef: so.number, newState: so, ip: req.ip });
...
router.get('/sales-orders', requireAuth, asyncHandler(async (req, res) => {
  let rows = await db.all(TID, COL.salesOrders);
...
router.get('/sales-orders/:id', requireAuth, asyncHandler(async (req, res) => {
  const so = notFoundIfUndefined(await db.byId(TID, COL.salesOrders, req.params.id), 'Sales order not found');
  res.json(so);
}));
router.put('/sales-orders/:id', requireAuth, requireRole('manager', 'admin', 'owner'), asyncHandler(async (req, res) => {
  const existing = notFoundIfUndefined(await db.byId(TID, COL.salesOrders, req.params.id), 'Sales order not found');
  const updated = await db.update(TID, COL.salesOrders, existing.id, req.body);
  const a = actor(req); await recordAudit({ tenantId: TID, actorId: a.id, actorName: a.name, action: 'update', module: 'crm', recordRef: existing.number, newState: updated, ip: req.ip });
  res.json(updated);
}));

async function init() {
  await db.seed(COL.customers, [
