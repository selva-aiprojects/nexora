...
docs.forEach((d) => db.seed(COL.documents, [{ ...d, createdAt: '2026-08-01', updatedAt: '2026-08-10' }]));
db.seed(COL.versions, [
...
// ----------------------------- Routes -----------------------------
router.get('/folders', requireAuth, asyncHandler(async (_req, res) => res.json(db.all(TID, COL.folders))));
router.get('/documents', requireAuth, asyncHandler(async (req, res) => {
  let rows = db.all(TID, COL.documents);
...
router.post('/documents', requireAuth, asyncHandler(async (req, res) => {
  ...
  const row = db.insert(TID, COL.documents, { ownerId: req.user!.id, tags: req.body.tags ?? [], mimeType: req.body.mimeType ?? 'application/pdf', size: req.body.size ?? 0, currentVersion: 1, createdAt: new Date().toISOString().slice(0, 10), updatedAt: new Date().toISOString().slice(0, 10), ...req.body });
  db.insert(TID, COL.versions, { documentId: row.id, version: 1, uploadedOn: row.createdAt, note: 'Initial upload' });
  const a = actor(req); recordAudit({ tenantId: TID, actorId: a.id, actorName: a.name, action: 'create', module: 'dms', recordRef: row.id, newState: row, ip: req.ip });
  res.status(201).json(row);
}));
router.get('/documents/:id', requireAuth, asyncHandler(async (req, res) => {
  const doc = notFoundIfUndefined(db.byId(TID, COL.documents, req.params.id), 'Document not found');
...
router.get('/documents/:id/versions', requireAuth, asyncHandler(async (req, res) => {
  res.json(db.query(TID, COL.versions, (v) => v.documentId === req.params.id).sort((a, b) => b.version - a.version));
}));
router.post('/documents/:id/versions', requireAuth, asyncHandler(async (req, res) => {
  const doc = notFoundIfUndefined(db.byId(TID, COL.documents, req.params.id), 'Document not found');
  const next = doc.currentVersion + 1;
  const ver = db.insert(TID, COL.versions, { documentId: doc.id, version: next, uploadedOn: new Date().toISOString().slice(0, 10), note: req.body?.note ?? 'New version' });
  db.update(TID, COL.documents, doc.id, { currentVersion: next, updatedAt: ver.uploadedOn });
  const a = actor(req); recordAudit({ tenantId: TID, actorId: a.id, actorName: a.name, action: 'update', module: 'dms', recordRef: doc.id, newState: { version: next }, ip: req.ip });
...
router.post('/documents/:id/share', requireAuth, asyncHandler(async (req, res) => {
  ...
  const doc = notFoundIfUndefined(db.byId(TID, COL.documents, req.params.id), 'Document not found');
  const row = db.insert(TID, COL.shares, { documentId: doc.id, expiresOn: req.body.expiresOn ?? null, ...req.body });
  const a = actor(req); recordAudit({ tenantId: TID, actorId: a.id, actorName: a.name, action: 'create', module: 'dms', recordRef: doc.id, newState: row, ip: req.ip });
  res.status(201).json(row);
}));
