...
router.get('/home', requireAuth, asyncHandler(async (req, res) => {
  ...
  const employee = notFoundIfUndefined(await db.byId(TID, EMP_COL, eid), 'Employee profile not found');
  const attendanceToday = await db.query(TID, ATT_COL, (a) => a.employeeId === eid && a.date === today);
  const attendance = attendanceToday[0] ?? null;
  const leaveBalances = await db.query(TID, 'hrms_leave_balances', (b) => b.employeeId === eid);
  const payslips = (await db.query(TID, PAY_COL, (p) => p.employeeId === eid)).sort((a, b) => b.period.localeCompare(a.period));
  const pendingLeave = (await db.query(TID, LEAVE_COL, (l) => l.employeeId === eid && l.status === 'pending')).length;
  const pendingExpenses = (await db.query(TID, EXP_COL, (e) => e.employeeId === eid && e.status === 'submitted')).length;
  const announcements = [
  ...
  res.json({
    ...
    attendanceStatus: attendance ? { checkedIn: true, checkIn: attendance.checkIn, status: attendance.status } : { checkedIn: false },
    leaveBalances,
...
router.post('/attendance/check-in', requireAuth, asyncHandler(async (req, res) => {
  const eid = empId(req);
  const today = new Date().toISOString().slice(0, 10);
  if ((await db.query(TID, ATT_COL, (a) => a.employeeId === eid && a.date === today))[0]) throw ApiError.conflict('Already checked in today');
  const row = await db.insert(TID, ATT_COL, { employeeId: eid, date: today, checkIn: new Date().toTimeString().slice(0, 5), checkOut: null, status: 'present', gps: req.body?.gps });
  res.status(201).json(row);
}));
router.post('/attendance/check-out', requireAuth, asyncHandler(async (req, res) => {
  const eid = empId(req);
  const today = new Date().toISOString().slice(0, 10);
  const rec = (await db.query(TID, ATT_COL, (a) => a.employeeId === eid && a.date === today))[0];
  if (!rec) throw ApiError.notFound('No check-in found for today');
  res.json(await db.update(TID, ATT_COL, rec.id, { checkOut: new Date().toTimeString().slice(0, 5) }));
}));

// Leave
router.get('/leave', requireAuth, asyncHandler(async (req, res) => {
  const eid = empId(req);
  const rows = await db.query(TID, LEAVE_COL, (l) => l.employeeId === eid);
...
router.post('/leave', requireAuth, asyncHandler(async (req, res) => {
  ...
  const row = await db.insert(TID, LEAVE_COL, { employeeId: eid, status: 'pending', reason: req.body.reason ?? '', ...req.body });
  const a = actor(req); await recordAudit({ tenantId: TID, actorId: a.id, actorName: a.name, action: 'create', module: 'ess', recordRef: row.id, newState: row, ip: req.ip });
  res.status(201).json(row);
}));
...
router.get('/expenses', requireAuth, asyncHandler(async (req, res) => {
  const eid = empId(req);
  let rows = await db.query(TID, EXP_COL, (e) => e.employeeId === eid);
...
router.post('/expenses', requireAuth, asyncHandler(async (req, res) => {
  ...
  const row = await db.insert(TID, EXP_COL, { employeeId: eid, status: 'submitted', description: req.body.description ?? '', ...req.body });
  const a = actor(req); await recordAudit({ tenantId: TID, actorId: a.id, actorName: a.name, action: 'create', module: 'ess', recordRef: row.id, newState: row, ip: req.ip });
  res.status(201).json(row);
}));
...
router.get('/payslips', requireAuth, asyncHandler(async (req, res) => {
  const eid = empId(req);
  const rows = (await db.query(TID, PAY_COL, (p) => p.employeeId === eid)).sort((a, b) => b.period.localeCompare(a.period));
  res.json(listResult(rows, rows.length, 1, rows.length));
}));
router.get('/documents', requireAuth, asyncHandler(async (req, res) => {
  const eid = empId(req);
  const rows = await db.query(TID, DOC_COL, (d) => d.ownerId === eid || d.category === 'policy');
  res.json(listResult(rows, rows.length, 1, rows.length));
}));

async function init() {
  await db.seed(EXP_COL, [
