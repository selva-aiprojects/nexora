...
router.get('/vendors', requireAuth, asyncHandler(async (req, res) => {
  let rows = await db.all(TID, COL.vendors);
...
router.post('/vendors', requireAuth, requireRole('manager', 'admin', 'owner'), asyncHandler(async (req, res) => {
  ...
  const row = await db.insert(TID, COL.vendors, { id: await db.nextId('ven', COL.vendors), createdAt: new Date().toISOString().slice(0, 10), ...req.body });
  const a = actor(req); await recordAudit({ tenantId: TID, actorId: a.id, actorName: a.name, action: 'create', module: 'procurement', recordRef: row.id, newState: row, ip: req.ip });
  res.status(201).json(row);
}));
router.get('/vendors/:id', requireAuth, asyncHandler(async (req, res) => {
  const vendor = notFoundIfUndefined(await db.byId(TID, COL.vendors, req.params.id), 'Vendor not found');
  res.json(vendor);
}));
router.put('/vendors/:id', requireAuth, requireRole('manager', 'admin', 'owner'), asyncHandler(async (req, res) => {
  const existing = notFoundIfUndefined(await db.byId(TID, COL.vendors, req.params.id), 'Vendor not found');
  const updated = await db.update(TID, COL.vendors, existing.id, req.body);
  const a = actor(req); await recordAudit({ tenantId: TID, actorId: a.id, actorName: a.name, action: 'update', module: 'procurement', recordRef: existing.id, newState: updated, ip: req.ip });
  res.json(updated);
}));

// Vendor Quotes
router.get('/vendor-quotes', requireAuth, asyncHandler(async (req, res) => {
  let rows = await db.all(TID, COL.vendorQuotes);
...
router.post('/vendor-quotes', requireAuth, requireRole('manager', 'admin', 'owner'), asyncHandler(async (req, res) => {
  ...
  const row = await db.insert(TID, COL.vendorQuotes, req.body);
  const a = actor(req); await recordAudit({ tenantId: TID, actorId: a.id, actorName: a.name, action: 'create', module: 'procurement', recordRef: row.id, newState: row, ip: req.ip });
  res.status(201).json(row);
}));
router.put('/vendor-quotes/:id', requireAuth, requireRole('manager', 'admin', 'owner'), asyncHandler(async (req, res) => {
  const existing = notFoundIfUndefined(await db.byId(TID, COL.vendorQuotes, req.params.id), 'Vendor quote not found');
  const updated = await db.update(TID, COL.vendorQuotes, existing.id, req.body);
  const a = actor(req); await recordAudit({ tenantId: TID, actorId: a.id, actorName: a.name, action: 'update', module: 'procurement', recordRef: existing.id, newState: updated, ip: req.ip });
  res.json(updated);
}));
...
router.get('/contracts', requireAuth, asyncHandler(async (req, res) => {
  let rows = await db.all(TID, COL.contracts);
...
router.post('/contracts', requireAuth, requireRole('manager', 'admin', 'owner'), asyncHandler(async (req, res) => {
  ...
  const row = await db.insert(TID, COL.contracts, req.body);
  const a = actor(req); await recordAudit({ tenantId: TID, actorId: a.id, actorName: a.name, action: 'create', module: 'procurement', recordRef: row.id, newState: row, ip: req.ip });
  res.status(201).json(row);
}));
router.put('/contracts/:id', requireAuth, requireRole('manager', 'admin', 'owner'), asyncHandler(async (req, res) => {
  const existing = notFoundIfUndefined(await db.byId(TID, COL.contracts, req.params.id), 'Contract not found');
  const updated = await db.update(TID, COL.contracts, existing.id, req.body);
  const a = actor(req); await recordAudit({ tenantId: TID, actorId: a.id, actorName: a.name, action: 'update', module: 'procurement', recordRef: existing.id, newState: updated, ip: req.ip });
  res.json(updated);
}));
...
router.get('/grns', requireAuth, asyncHandler(async (req, res) => {
  let rows = await db.all(TID, COL.grns);
...
router.post('/grns', requireAuth, requireRole('manager', 'admin', 'owner'), asyncHandler(async (req, res) => {
  ...
  const row = await db.insert(TID, COL.grns, { id: await db.nextId('grn', COL.grns), ...req.body });
  const a = actor(req); await recordAudit({ tenantId: TID, actorId: a.id, actorName: a.name, action: 'create', module: 'procurement', recordRef: row.id, newState: row, ip: req.ip });
  res.status(201).json(row);
}));
router.put('/grns/:id', requireAuth, requireRole('manager', 'admin', 'owner'), asyncHandler(async (req, res) => {
  const existing = notFoundIfUndefined(await db.byId(TID, COL.grns, req.params.id), 'GRN not found');
  const updated = await db.update(TID, COL.grns, existing.id, req.body);
  const a = actor(req); await recordAudit({ tenantId: TID, actorId: a.id, actorName: a.name, action: 'update', module: 'procurement', recordRef: existing.id, newState: updated, ip: req.ip });
  res.json(updated);
}));

async function init() {
  await db.seed(COL.vendors, [
  ...
  await db.seed(COL.vendorQuotes, [
