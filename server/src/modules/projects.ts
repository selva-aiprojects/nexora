...
router.get('/projects', asyncHandler(async (req, res) => {
  ...
  let rows = await db.all(tenantId(req), COL.projects);
...
router.get('/projects/:id', asyncHandler(async (req, res) => {
  const project = await db.byId(tenantId(req), COL.projects, req.params.id);
...
router.post('/projects', asyncHandler(async (req, res) => {
  ...
  const project = await db.insert(tenantId(req), COL.projects, {
  ...
  const a = actor(req);
  await recordAudit({ tenantId: TID, actorId: a.id, actorName: a.name, action: 'create', module: 'projects', recordRef: project.id, newState: project, ip: req.ip });
  res.status(201).json(project);
}));

router.put('/projects/:id', asyncHandler(async (req, res) => {
  const existing = await db.byId(tenantId(req), COL.projects, req.params.id);
  notFoundIfUndefined(existing, 'Project not found');
  const updated = await db.update(tenantId(req), COL.projects, req.params.id, {
    ...req.body,
    budget: req.body.budget !== undefined ? Number(req.body.budget) : existing.budget,
  });
  const a = actor(req);
  await recordAudit({ tenantId: TID, actorId: a.id, actorName: a.name, action: 'update', module: 'projects', recordRef: updated.id, oldState: existing, newState: updated, ip: req.ip });
  res.json(updated);
}));
...
router.delete('/projects/:id', asyncHandler(async (req, res) => {
  const existing = await db.byId(tenantId(req), COL.projects, req.params.id);
  notFoundIfUndefined(existing, 'Project not found');
  await db.remove(tenantId(req), COL.projects, req.params.id);
  const a = actor(req);
  await recordAudit({ tenantId: TID, actorId: a.id, actorName: a.name, action: 'delete', module: 'projects', recordRef: req.params.id, oldState: existing, ip: req.ip });
  res.status(204).send();
}));

router.get('/wbs', asyncHandler(async (req, res) => {
  const projectId = req.query.projectId as string | undefined;
  let rows = await db.all(tenantId(req), COL.wbs);
...
router.post('/wbs', asyncHandler(async (req, res) => {
  ...
  const wbs = await db.insert(tenantId(req), COL.wbs, {
  ...
  const a = actor(req);
  await recordAudit({ tenantId: TID, actorId: a.id, actorName: a.name, action: 'create', module: 'projects', recordRef: wbs.id, newState: wbs, ip: req.ip });
  res.status(201).json(wbs);
}));

router.put('/wbs/:id', asyncHandler(async (req, res) => {
  const existing = await db.byId(tenantId(req), COL.wbs, req.params.id);
  notFoundIfUndefined(existing, 'WBS item not found');
  const updated = await db.update(tenantId(req), COL.wbs, req.params.id, req.body);
  const a = actor(req);
  await recordAudit({ tenantId: TID, actorId: a.id, actorName: a.name, action: 'update', module: 'projects', recordRef: updated.id, oldState: existing, newState: updated, ip: req.ip });
  res.json(updated);
}));
...
router.get('/time-entries', asyncHandler(async (req, res) => {
  const projectId = req.query.projectId as string | undefined;
  let rows = await db.all(tenantId(req), COL.timeEntries);
  if (projectId) rows = rows.filter((r: any) => r.projectId === projectId);
  res.json(listResult(rows, rows.length, 1, rows.length));
}));
...
router.post('/time-entries', asyncHandler(async (req, res) => {
  ...
  const entry = await db.insert(tenantId(req), COL.timeEntries, {
  ...
  const a = actor(req);
  await recordAudit({ tenantId: TID, actorId: a.id, actorName: a.name, action: 'create', module: 'projects', recordRef: entry.id, newState: entry, ip: req.ip });
  res.status(201).json(entry);
}));

router.get('/budgets', asyncHandler(async (req, res) => {
  const projectId = req.query.projectId as string | undefined;
  let rows = await db.all(tenantId(req), COL.budgets);
  if (projectId) rows = rows.filter((r: any) => r.projectId === projectId);
  res.json(listResult(rows, rows.length, 1, rows.length));
}));
...
router.post('/budgets', asyncHandler(async (req, res) => {
  ...
  const budget = await db.insert(tenantId(req), COL.budgets, {
    projectId, category, amount: Number(amount), spent: 0, period, createdAt: new Date().toISOString(),
  });
  const a = actor(req);
  await recordAudit({ tenantId: TID, actorId: a.id, actorName: a.name, action: 'create', module: 'projects', recordRef: budget.id, newState: budget, ip: req.ip });
  res.status(201).json(budget);
}));

router.get('/reports/project-pl', asyncHandler(async (req, res) => {
  const projectId = req.query.projectId as string | undefined;
  const projects = projectId ? await db.query(tenantId(req), COL.projects, (r: any) => r.id === projectId) : await db.all(tenantId(req), COL.projects);
  const report: any[] = [];
  for (const p of projects) {
    const wbs = await db.query(tenantId(req), COL.wbs, (r: any) => r.projectId === p.id);
    const entries = await db.query(tenantId(req), COL.timeEntries, (r: any) => r.projectId === p.id);
    const revenue = p.budget ?? 0;
    const cost = entries.reduce((s: number, e: any) => s + (e.hours * 1500), 0);
    report.push({ projectId: p.id, name: p.name, revenue, cost, profit: revenue - cost, wbsCount: wbs.length, timeEntries: entries.length });
  }
